...
export class ClaudeRuntimeGateway implements OnModuleInit, OnModuleDestroy {
  ...
  private async handleMessage(sessionId: number, ws: WebSocket, rawMessage: string): Promise<void> {
    let action: ClaudeRuntimeClientAction;
    try {
      action = JSON.parse(rawMessage) as ClaudeRuntimeClientAction;
    } catch {
      ws.send(JSON.stringify({ type: 'error', payload: { sessionId, message: 'Invalid message payload' } }));
      return;
    }
    ...
    try {
      switch (action.type) {
        ...
        case 'submit_prompt':
          await this.runtimeService.submitPrompt(sessionId, action.prompt);
          return;
        case 'interrupt':
