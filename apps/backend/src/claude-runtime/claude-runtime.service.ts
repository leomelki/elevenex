...
export class ClaudeRuntimeService extends EventEmitter {
  ...
  async submitPrompt(
    ...
    try {
      ...
      // lastHistorySource=null. Resuming with a stale ID causes Claude Code to return
      // "No conversation found" and the user's first message is lost. Clear the ID now
      ...
      this.logger.log(
        `Claude session title eligibility checked session=${sessionId} hasClaudeSessionId=${Boolean(session.claudeSessionId && session.claudeSessionId !== '-1')} currentName=${JSON.stringify(session.name)} shouldGenerate=${shouldGenerateSessionTitle}`,
      );
      ...
      void Promise.resolve(
        this.sessionsService.updateStatus(sessionId, 'active'),
      )
        .then(() => {
      ...
      try {
        ...
        // Keep the initialization guard clear across normal, interrupted, and
        // runtime-error paths after activeRuns registration.
        this.initializingRuns.delete(sessionId);
        const run = this.activeRuns.get(sessionId);
      ...
      if (!runRegistered && initializationState) {
        ...
        initializationState.sessionState = 'idle';
        initializationState.canInterrupt = false;
        this.emitEvent({ type: 'error', payload: { sessionId, message } });
        this.emitRunState(sessionId);
        void this.claudeHooksService.updateStatus(sessionId, 'idle', {
          markCompletion: false,
  ...
  private async queuePendingPrompt(
    sessionId: number,
    prompt: string,
    images: ClaudeImageInput[],
  ): Promise<void> {
  ...
  private async generateAndSaveSessionTitle(
    ...
    this.logger.log(
      `Claude session title generation saved session=${sessionId} title=${JSON.stringify(title)} persistedName=${JSON.stringify(updated?.name ?? null)}`,
    );
  }
