...
export type ClaudeSessionExecutionState =
  | 'idle'
  | 'running'
  | 'requires_action'
  | null;
...
export interface ClaudeModelOption {
  ...
  /**
   * Thinking levels this specific model accepts, when the provider tells us.
   * Absent means "unknown" — fall back to the provider-wide list.
   */
...
export interface ClaudeSessionSnapshotPayload extends ClaudeRuntimeStatePayload {
  history: ClaudeTranscriptItem[];
}

export type ClaudeRuntimeEvent =
  | { type: 'session_snapshot'; payload: ClaudeSessionSnapshotPayload }
  | { type: 'runtime_snapshot'; payload: ClaudeRuntimeStatePayload }
  | {
      type: 'history_snapshot';
      payload: { sessionId: number; history: ClaudeTranscriptItem[] };
    }
  | {
  ...
  | {
      ...
      payload: { sessionId: number; claudeSessionId: string };
    }
  | {
      type: 'run_state';
      payload: {
  ...
  | {
      type: 'runtime_status';
      payload: { sessionId: number; status: ClaudeRuntimeStatus };
    }
  | {
      type: 'auth_status';
      payload: { sessionId: number; status: ClaudeAuthStatus };
    }
  | {
      type: 'rate_limit';
  ...
  | {
      type: 'notification';
      payload: { sessionId: number; notification: ClaudeNotification };
    }
  ...
  | {
      type: 'hook_event';
      payload: { sessionId: number; hookEvent: ClaudeHookEvent };
    }
  | {
      type: 'hook_started';
      payload: { sessionId: number; hook: ClaudeHookExecution };
    }
  | {
      type: 'hook_progress';
      payload: { sessionId: number; hook: ClaudeHookExecution };
  ...
  | {
      type: 'task_started';
      payload: { sessionId: number; task: ClaudeTaskState };
    }
  | {
      type: 'task_updated';
      payload: { sessionId: number; task: ClaudeTaskState };
  ...
  | {
      type: 'task_lifecycle';
      payload: { sessionId: number; taskLifecycle: ClaudeTaskLifecycle };
    }
  | {
  ...
  | {
      type: 'tool_progress';
      payload: { sessionId: number; progress: ClaudeToolProgress };
    }
  | {
      type: 'tool_summary';
      payload: { sessionId: number; summary: ClaudeToolUseSummary };
    }
  | {
      type: 'memory_recall';
      payload: { sessionId: number; recall: ClaudeMemoryRecall };
    }
  | {
      type: 'files_persisted';
      payload: { sessionId: number; files: ClaudeFilesPersisted };
    }
  | {
  ...
  | {
      ...
      payload: { sessionId: number; boundary: ClaudeCompactBoundary };
    }
  | {
      type: 'mirror_error';
      payload: { sessionId: number; error: ClaudeMirrorError };
    }
  | {
      type: 'message_start';
      payload: { sessionId: number; item: ClaudeTranscriptItem };
    }
  | {
      type: 'message_delta';
      payload: { sessionId: number; itemId: string; delta: string };
    }
  ...
  | {
      ...
      payload: { sessionId: number; itemId: string };
    }
  | {
      type: 'tool_use';
      payload: { sessionId: number; item: ClaudeTranscriptItem };
  ...
  | {
      ...
      payload: { sessionId: number; request: ClaudePermissionRequest };
    }
  | {
  ...
  | {
      ...
      payload: { sessionId: number; request: ClaudeUserInputRequest };
    }
