...
export class CodexRuntimeService extends EventEmitter {
  ...
  async submitPrompt(
    ...
    const shouldLoadSessionForTitle =
      !worktreePath || !cachedState?.codexSessionId;
    if (shouldLoadSessionForTitle) {
      ...
      this.logger.log(
        `Codex session title eligibility checked session=${sessionId} source=${hadCachedWorktreePath ? 'cached_state_refresh' : 'session_load'} hasCodexSessionId=${Boolean(session.codexSessionId && session.codexSessionId !== '-1')} currentName=${JSON.stringify(session.name)} shouldGenerate=${isNewSession}`,
      );
    } else {
      this.logger.debug(
        `Codex session title generation skipped session=${sessionId} reason=existing_codex_session`,
      );
    }
    if (!worktreePath) {
    ...
    if (isNewSession) {
      ...
      if (effectiveTitlePrompt) {
        this.logger.log(
          `Codex session title generation scheduled session=${sessionId} worktreePath=${JSON.stringify(resolvedWorktreePath)} promptLength=${effectiveTitlePrompt.length}`,
        );
        setImmediate(() => {
        ...
        this.logger.warn(
          `Codex session title generation skipped session=${sessionId} reason=empty_prompt`,
        );
      }
    }
    ...
    try {
      ...
      for await (const event of this.runTurnOnAppServer(
        ...
        this.handleCodexEvent(sessionId, state, event, resolvedWorktreePath);
      }
  ...
  private async generateAndSaveSessionTitle(
    ...
    try {
      ...
      this.logger.log(
        `Codex session title generation saved session=${sessionId} title=${JSON.stringify(title)} persistedName=${JSON.stringify(updated?.name ?? null)}`,
      );
    } catch (error) {
      this.logger.warn(
  ...
  private toRuntimeStatePayload(
    ...
    return {
      ...
      warmState:
        this.activeRuns.has(sessionId) || this.lastPrewarmAt.has(sessionId)
          ? 'warm'
          : 'cold',
  ...
  private toCodexUserInputQuestions(
    ...
    type CodexUserInputQuestion = NonNullable<
