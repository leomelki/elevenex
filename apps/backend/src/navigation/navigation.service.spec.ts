...
import { WorkspacesService, WorkspaceSnapshot } from '../workspaces/workspaces.service.js';
...
describe('NavigationService', () => {
  ...
  const repo = { id: 1, projectId: 1, name: 'repo-1', path: '/path/to/repo', color: null, preferredContextRootRef: null, createdAt: '2024-01-01' };
  ...
  beforeEach(async () => {
    mockProjectsService = { findAll: jest.fn() } as unknown as jest.Mocked<ProjectsService>;
    mockReposService = { findByProject: jest.fn() } as unknown as jest.Mocked<ReposService>;
    mockSessionsService = { findByRepo: jest.fn() } as unknown as jest.Mocked<SessionsService>;
    mockWorkspacesService = { listForRepo: jest.fn() } as unknown as jest.Mocked<WorkspacesService>;
  ...
  it('groups live and archived sessions under workspaces', async () => {
    ...
    mockWorkspacesService.listForRepo.mockResolvedValue([
      ...
      workspace({ id: 2, name: 'Review', path: '/path/review', currentBranch: 'feature', isDefault: false }),
    ]);
    mockSessionsService.findByRepo.mockResolvedValue([
      session({ id: 1, workspaceId: 1, name: 'Live', status: 'active', branchName: 'main', worktreePath: '/path/to/repo' }),
      session({ id: 2, workspaceId: 1, name: 'Archived', status: 'archived', branchName: 'main', worktreePath: '/path/to/repo' }),
      session({ id: 3, workspaceId: 2, name: 'Feature', status: 'created', branchName: 'feature', worktreePath: '/path/review' }),
    ]);

    const result = await service.getNavigationTree();

    expect(result[0].repos[0].workspaces).toHaveLength(2);
    const defaultWorkspace = result[0].repos[0].workspaces.find((item) => item.name === 'Default');
    const reviewWorkspace = result[0].repos[0].workspaces.find((item) => item.name === 'Review');

    expect(defaultWorkspace?.sessions.map((item) => item.id)).toEqual([1]);
    expect(defaultWorkspace?.archivedSessions.map((item) => item.id)).toEqual([2]);
  ...
  it('keeps legacy path-based sessions attached when workspaceId is missing', async () => {
    ...
    mockReposService.findByProject.mockResolvedValue([repo]);
    mockWorkspacesService.listForRepo.mockResolvedValue([
      workspace({ id: 2, name: 'Legacy', path: '/path/legacy', currentBranch: 'legacy', isDefault: false }),
    ]);
    mockSessionsService.findByRepo.mockResolvedValue([
      session({ id: 4, workspaceId: null, branchName: 'legacy', worktreePath: '/path/legacy' }),
    ]);
  ...
  it('marks repo errors when workspace reconciliation fails', async () => {
    ...
    mockReposService.findByProject.mockResolvedValue([repo]);
    mockSessionsService.findByRepo.mockResolvedValue([]);
    mockWorkspacesService.listForRepo.mockRejectedValue(new Error('Path not found'));
