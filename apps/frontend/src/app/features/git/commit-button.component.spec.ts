...
describe('CommitButtonComponent', () => {
  ...
  it('keeps the trigger visible while committing even after changes are cleared', async () => {
    ...
    fixture.componentInstance.commitMessage.set('fix: something');

    const commitResponse = new Subject<{
      hash: string;
      message: string;
      generatedMessage: boolean;
    }>();
    gitServiceMock.commit.mockReturnValueOnce(commitResponse.asObservable());

    const commitPromise = fixture.componentInstance.submitCommit();
    fixture.detectChanges();
  ...
  it('does not show another worktree as committing while a commit is still in flight', async () => {
    ...
    expect(fixture.nativeElement.textContent).not.toContain('Committing');
