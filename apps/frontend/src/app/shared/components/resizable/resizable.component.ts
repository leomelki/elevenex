...
export class ZardResizableComponent implements AfterContentInit, OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (this.initialized && count > 0 && count !== prevCount) {
        prevCount = count;
  ...
  startResize(handleIndex: number, event: MouseEvent | TouchEvent): void {
    ...
    // Clean up any leaked listeners from a previous incomplete resize
    this.listenersCleanup?.();
    ...
    if (this.isBrowser) {
      ...
      this.document.body.style.cursor = layout === 'vertical' ? 'row-resize' : 'col-resize';
