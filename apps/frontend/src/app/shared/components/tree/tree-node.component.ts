      ...
      <div role="group" [class]="childrenClasses()">
        ...
        @for (child of node().children; track child.key; let i = $index) {
          <z-tree-node
            ...
            role="treeitem"
            [attr.aria-expanded]="!child.leaf ? treeService.isExpanded(child.key) : null"
...
export class ZardTreeNodeComponent<T> {
  ...
  onToggle(event: Event) {
    ...
    this.treeService.toggle(this.node().key, this.node());
  }
  ...
  onContentClick() {
    ...
    // Toggle expansion for non-leaf nodes (directories)
    if (!this.node().leaf) {
      this.treeService.toggle(this.node().key, this.node());
    }
  }
