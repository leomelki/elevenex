    ...
    @if (zVirtualScroll()) {
      ...
      @for (node of zData(); track node.key; let i = $index) {
        <z-tree-node
          ...
          [nodeTemplate]="customNodeTemplate() ?? null"
          role="treeitem"
          [attr.aria-expanded]="!node.leaf ? treeService.isExpanded(node.key) : null"
          [attr.aria-level]="1"
...
export class ZardTreeComponent<T> {
  ...
  constructor() {
    ...
    // Emit expand events
    effect(() => {
      const expanded = this.treeService.expandedNode();
      if (expanded) {
        this.zNodeExpand.emit(expanded.node);
      }
    });

    // Emit collapse events
    effect(() => {
      const collapsed = this.treeService.collapsedNode();
      if (collapsed) {
        this.zNodeCollapse.emit(collapsed.node);
      }
    });
  }
  ...
  private expandFocusedNode() {
    ...
    if (current?.expandable && !this.treeService.isExpanded(current.node.key)) {
      this.treeService.expand(current.node.key, current.node);
    }
  }
  ...
  private collapseFocusedNode() {
    ...
    if (current && this.treeService.isExpanded(current.node.key)) {
      this.treeService.collapse(current.node.key, current.node);
    }
  }
