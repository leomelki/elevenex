...
export class ZardTreeService<T = any> {
  ...
  // Expand/collapse notification from tree-node → tree component
  private _expandId = 0;
  readonly expandedNode = signal<{ node: TreeNode<T>; _id: number } | null>(null);
  readonly collapsedNode = signal<{ node: TreeNode<T>; _id: number } | null>(null);

  notifyNodeClick(node: TreeNode<T>) {
    this.clickedNode.set({ node, _id: ++this._clickId });
  }

  notifyNodeExpand(node: TreeNode<T>) {
    this.expandedNode.set({ node, _id: ++this._expandId });
  }

  notifyNodeCollapse(node: TreeNode<T>) {
    this.collapsedNode.set({ node, _id: ++this._expandId });
  }
  ...
  readonly flattenedNodes = computed(() => {
    ...
    const flatten = (nodes: TreeNode<T>[], level: number) => {
      for (const node of nodes) {
        const expandable = !node.leaf;
        result.push({ node, level, expandable, index: index++ });
        if (expandable && node.children?.length && this.expandedKeys().has(node.key)) {
  ...
  toggle(key: string, node?: TreeNode<T>) {
    if (this.isExpanded(key)) {
      this.collapse(key, node);
    } else {
      this.expand(key, node);
    }
  }

  expand(key: string, node?: TreeNode<T>) {
    ...
    if (node) {
      this.notifyNodeExpand(node);
    }
  }

  collapse(key: string, node?: TreeNode<T>) {
    this.expandedKeys.update(keys => {
      const next = new Set(keys);
      next.delete(key);
      return next;
